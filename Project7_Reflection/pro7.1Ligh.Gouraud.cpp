...
{
    renderingProgram = Utils::createShaderProgram("GLSL/GouraudShaders/vertShader.glsl", "GLSL/GouraudShaders/fragShader.glsl");
    cameraX = 0.0f;
    cameraY = 0.0f;
    cameraZ = 8.0f;
    sphLocX = 0.0f;
...
void display(GLFWwindow *window, double currentTime)
{
    ...
    cameraRotateMat = glm::rotate(glm::mat4(1.0f), cameraRotateAlpha, glm::vec3(1.0f, 0.0f, 0.0f));
    cameraRotateMat = glm::rotate(cameraRotateMat, cameraRotateBeta, glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    if (mvRotate)
        mMat = glm::scale(mMat, glm::vec3(1.0f, 1.0f, 1.0f)) * cameraRotateMat;
    else
...
void window_CursorPos_callback(GLFWwindow *window, double x, double y)
{
    ...
    if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS)
    {
        ...
        else
        {
            ...
            {
                cameraRotateAlpha += Utils::normalize(y - lastY, -3.14f, 3.14f) * 0.01f;
                cameraRotateBeta += Utils::normalize(x - lastX, -3.14f, 3.14f) * 0.01f;
                lastX = x;
                lastY = y;
            }
        }
